// Peter's changes to Tomasz's original are marked with "PGL" and summarized below:
//
// The target function has changed from (*ptfunc)(float *x,int n) to (*ptfunc)(float *x,int *n)
// ("int n" to "int *n") so that only pointers are passed to the Fortran target function.
// This has to do with Fortran being pass-by-reference (and C pointers being essentially references).
//
...
struct psobird *b;
//extern struct view v_; //structure used for communication with fortran // PGL commented
struct timeval t; 
int i,j,iter,cl;
char pomm[80];
gsl_rng * R;
