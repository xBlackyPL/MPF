...
// fort_ means that we intend to use C function in fortran (note wrapped
// pointers)
float fort_pso_(float *x, int *pn, int *pnb, float *min, float *max,
                float *peps, int *maxiter, float *xtemp, float *ftemp);

float c_t_func(float *x, int n) { return t_func_(&x[0], &n); }
