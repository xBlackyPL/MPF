...
float grosen(float *x, int n) {
    ...
    f = 0.0f;
    for (j = 0; j < n - 1; j++) {
        f = f + 100.0f * powf(x[j + 1] - x[j] * x[j], 2.0f) +
            powf(1.0f - x[j], 2.0f);
    }
...
int main() {
    ...
    // regular parallel call (based on OMP_NUM_THREADS) uncomment it for regular
    // application f=pso(x,n,&grosen,nb,min,max,eps,&maxiter,&xtemp[0],&ftemp);
    ...
    // xtemp, ftem -> temporal best solution and corresponding target function
    // value
...
#pragma omp parallel
    ...
    omp_set_num_threads(
        2);    // two threads will be started in the next parallel section
    flag = 0;  // control flag (has function exited?)
#pragma omp parallel
    {
        if (omp_get_thread_num() ==
            0)  // thread 0. Does real work using nt number of threads
        {
            omp_set_num_threads(nt);
            f = pso(x, n, &grosen, nb, min, max, eps, &maxiter, &xtemp[0],
                    &ftemp);
            flag = 1;  // function has exited (leave parallel section)
        } else         // control part (thread 1)
        {
            while (
                1)  // every 1 second dump temporal information about solution.
            {
                sleep(1);
                for (j = 0; j < n; j++) {
                ...
                if (ftemp < 0.01)  // useful trick. Use maxiter to force stop
                                   // the fuction when some previously unknown
